...
const KEYWORD_DICT = require("../src/utils/tag-keywords.json");
...
const SEARCH_BATCHES = [
  {
    label: "PK 2000-Jun2014",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2000-01-01..2014-06-30",
  },
  {
    label: "PK Jul2014-Jan2016",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2014-07-01..2016-01-31",
  },
  {
    label: "PK Feb2016-Feb2017",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2016-02-01..2017-02-28",
  },
  {
    label: "PK Mar2017-Dec2017",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2017-03-01..2017-12-31",
  },
  {
    label: "PK Jan2018-Sep2018",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2018-01-01..2018-09-30",
  },
  {
    label: "PK Oct2018-Apr2019",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2018-10-01..2019-04-30",
  },
  {
    label: "PK May2019-Sep2019",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2019-05-01..2019-09-30",
  },
  {
    label: "PK Oct2019-Feb2020",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2019-10-01..2020-02-29",
  },
  {
    label: "PK Mar2020-Jun2020",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2020-03-01..2020-06-30",
  },
  {
    label: "PK Jul2020-Nov2020",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2020-07-01..2020-11-30",
  },
  {
    label: "PK Dec2020-Mar2021",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2020-12-01..2021-03-31",
  },
  {
    label: "PK Apr2021-Aug2021",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2021-04-01..2021-08-31",
  },
  {
    label: "PK Sep2021-Dec2021",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2021-09-01..2021-12-31",
  },
  {
    label: "PK Jan2022-Apr2022",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2022-01-01..2022-04-30",
  },
  {
    label: "PK May2022-Aug2022",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2022-05-01..2022-08-31",
  },
  {
    label: "PK Sep2022-Nov2022",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2022-09-01..2022-11-30",
  },
  {
    label: "PK Dec2022-Feb2023",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2022-12-01..2023-02-28",
  },
  {
    label: "PK Mar2023-Jun2023",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2023-03-01..2023-06-30",
  },
  {
    label: "PK Jul2023-Sep2023",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2023-07-01..2023-09-30",
  },
  {
    label: "PK Oct2023-Dec2023",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2023-10-01..2023-12-31",
  },
  {
    label: "PK Jan2024-Mar2024",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2024-01-01..2024-03-31",
  },
  {
    label: "PK Apr2024-Jul2024",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2024-04-01..2024-07-31",
  },
  {
    label: "PK Aug2024-Dec2024",
    q: "location:pakistan type:user repos:>3 followers:>1 created:2024-08-01..2024-12-31",
  },
  {
    label: "PK 2025+",
...
function computeActivityMetrics(rawEvents) {
  ...
  const meaningful = (rawEvents || []).filter((e) =>
    MEANINGFUL_EVENT_TYPES.has(e.type),
  );
  ...
  if (timestamps.length > 0) {
    ...
    const gapToNow = Math.floor(
      (now - timestamps[timestamps.length - 1]) / (1000 * 60 * 60 * 24),
    );
...
function buildRegex(keyword) {
  try {
    ...
    console.warn(
      `Failed to build tag regex for keyword ${String(keyword)}: ${error.message}`,
    );
    return null;
  }
}
...
async function githubRequest(endpoint, token, options = {}) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error(
        `Request timeout after ${REQUEST_TIMEOUT_MS}ms for ${endpoint}`,
      );
    }
  ...
  if (response.status === 429 && !retriedAfter429) {
    ...
    const waitMs =
      (retryAfter > 0 ? retryAfter * 1000 : RATE_LIMIT_RETRY_DELAY_MS) + 5000;
    console.warn(
      `Rate limited on ${endpoint}; waiting ${Math.ceil(waitMs / 1000)}s and retrying.`,
    );
    await sleep(waitMs);
  ...
  if (!response.ok) {
    ...
    if (
      ...
      const waitMs = Math.max(0, rateLimit.resetAt * 1000 - Date.now()) + 5000;
      console.warn(
        `Rate limited (403) on ${endpoint}; waiting ${Math.ceil(waitMs / 1000)}s and retrying.`,
      );
      await sleep(waitMs);
      return githubRequest(endpoint, token, {
        ...options,
    ...
    throw new Error(
      `GitHub API error ${response.status} for ${endpoint}: ${body}`,
    );
  }
...
async function discoverUsers(token, batches) {
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    if (queries.length === 0) {
      console.warn(
        `[discover ${index + 1}/${SEARCH_BATCHES.length}] ${batch.label}: no search query; skipping`,
      );
      continue;
    }
    ...
    for (let qIdx = 0; qIdx < queries.length; qIdx += 1) {
      ...
      if (shardSuccess) {
        ...
        console.log(
          `  -> shard ${qIdx + 1}/${queries.length}: ${shardCount} users`,
        );
      }
...
function isLikelyFakeFromProfile(profile) {
  ...
  const hasNoNetwork =
    (profile.followers || 0) === 0 && (profile.following || 0) === 0;
...
function buildDigestRepos(reposActive7d, repos) {
  if (
    !Array.isArray(reposActive7d) ||
    reposActive7d.length === 0 ||
    !Array.isArray(repos) ||
    repos.length === 0
  ) {
    return [];
...
async function fetchDeveloperActivity(username, token) {
  ...
  const meaningfulLast30Days = filterMeaningfulEvents(
    extractRecentEvents(events, 30),
  );
  const repoSummary = summarizeRepos(repos);
  const reposActive7d = extractReposPushedInLast7Days(recentEvents).map(
    (repo) => repo.name,
  );
...
function applyActivityFilter(developers) {
  return developers.filter((dev) => {
    const hasEnoughContributions =
      dev.total_contributions_60d >= ACTIVITY_THRESHOLDS.MIN_CONTRIBUTIONS_60D;
    const hasNoLongGaps =
      dev.longest_gap_days <= ACTIVITY_THRESHOLDS.MAX_INACTIVITY_GAP_DAYS;
...
async function fetchPakistaniDevelopers(options = {}) {
  ...
  const token =
    options.token || process.env.MY_GITHUB_PAT || process.env.GITHUB_TOKEN;
  if (!token) {
    throw new Error(
      "Missing GitHub token. Set MY_GITHUB_PAT in .env or pass options.token.",
    );
  }
  ...
  const batches =
    batchIndex != null ? [SEARCH_BATCHES[batchIndex]] : SEARCH_BATCHES;
  ...
  const allUsernames = dedupeUsernames([
    ...discoveredUsers,
    ...registeredUsers,
  ]);
  ...
  for (let index = 0; index < allUsernames.length; index += 1) {
    ...
    if (index < allUsernames.length - 1) {
      await sleep(
        randomDelayMs(USER_CALL_DELAY_MIN_MS, USER_CALL_DELAY_MAX_MS),
      );
    }
  }
  ...
  if (rawOnly) {
    console.log(
      `Raw mode: returning ${fetchedDevelopers.length} unfiltered developers.`,
    );
  ...
  console.log(
    `Final leaderboard: ${finalDevelopers.length} developers (capped at ${MAX_DEVELOPERS})`,
  );
